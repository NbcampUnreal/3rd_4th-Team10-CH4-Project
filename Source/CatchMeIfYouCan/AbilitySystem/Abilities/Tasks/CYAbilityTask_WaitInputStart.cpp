#include "CYAbilityTask_WaitInputStart.h"

#include "AbilitySystemComponent.h"

UCYAbilityTask_WaitInputStart::UCYAbilityTask_WaitInputStart(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	
}

UCYAbilityTask_WaitInputStart* UCYAbilityTask_WaitInputStart::WaitInputStart(UGameplayAbility* OwningAbility)
{
	UCYAbilityTask_WaitInputStart* Task = NewAbilityTask<UCYAbilityTask_WaitInputStart>(OwningAbility);
	return Task;
}
...
*/
void UCYAbilityTask_WaitInputStart::Activate()
{
	...
	if (ASC && Ability)
	{
		...
		// 이 이벤트는 CYAbilitySystemComponent::AbilitySpecInputStarted()에서 발생
		// 플레이어가 새로운 입력을 시작할 때 트리거됨
		DelegateHandle = ASC->AbilityReplicatedEventDelegate(EAbilityGenericReplicatedEvent::GameCustom1, GetAbilitySpecHandle(), GetActivationPredictionKey()).AddUObject(this, &UCYAbilityTask_WaitInputStart::OnStartCallback);
...
void UCYAbilityTask_WaitInputStart::OnStartCallback()
{
	UAbilitySystemComponent* ASC = AbilitySystemComponent.Get();
