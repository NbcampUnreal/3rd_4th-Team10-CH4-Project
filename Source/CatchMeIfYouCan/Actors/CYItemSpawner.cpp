...
void ACYItemSpawner::BeginPlay()
{
    ...
    if (ACYItemSpawnManager* Manager = ACYItemSpawnManager::GetInstance(GetWorld()))
    {
        ...
        // 초기 Threshold 받기
        CurrentThreshold = Manager->GetCurrentThreshold();
    }
...
{
    // Spawner 자체 Threshold 업데이트
    CurrentThreshold = NewThreshold;
    
    UE_LOG(LogTemp, Warning, TEXT("Spawner received threshold change: %d"), CurrentThreshold);
...
void ACYItemSpawner::TrySpawnItem()
{
    ...
    // 자체 저장된 Threshold 사용
    UE_LOG(LogTemp, Warning, TEXT("=== TrySpawnItem: Threshold=%d, RemainingTime=%.1f ==="), 
           CurrentThreshold, GameState->GetMatchRemainingTimeLocal());
    
    FItemSpec SelectedSpec = Manager->SharedSpawnData->SelectRandomItemForThreshold(CurrentThreshold);

    UE_LOG(LogTemp, Warning, TEXT("Selected Spec: Primary=%.1f, Duration=%.1f"), 
